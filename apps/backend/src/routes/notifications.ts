...
notifications.get('/', requireAuth(), rateLimit(), async (c) => {
  ...
  const result = rows.map((n) => {
    ...
    const itemId =
      ...
      (data.lostItemId as string | undefined) ??
    ...
    if (n.type === 'match_found') type = 'match_found';
    else if (n.type === 'claim_submitted' || n.type === 'claim_approved') type = 'claim_update';
    else if (n.type === 'ucard_found') type = 'ucard_found';
    else type = 'item_resolved';
