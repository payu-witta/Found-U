...
import { reportLostUCardSchema } from '../utils/validators.js';
 ...
 * Report a lost UCard with SPIRE ID (8 digits). Stored as Argon2 hash only.
 */
ucard.post(
  '/report-lost',
  requireAuth(),
  rateLimit({ max: 5, windowMs: 60 * 60 * 1000 }),
  validate('json', reportLostUCardSchema),
  async (c) => {
    const user = c.get('user');
    const { spireId } = c.req.valid('json');

    const report = await reportLostUCard({
      userId: user.sub!,
      spireId,
    });
