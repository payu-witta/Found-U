...
import { CreditCard, CheckCircle, AlertCircle, Search, Shield } from "lucide-react";
...
import { submitFoundUCard, reportLostUCard } from "@/lib/api/ucard";
import type { UCardSubmitResult } from "@/lib/api/ucard";
...
export default function UCardPage() {
  ...
  // Found UCard state
  const [submitResult, setSubmitResult] = useState<UCardSubmitResult | null>(null);

  // Lost UCard state
  const [spireId, setSpireId] = useState("");
  const [reportSuccess, setReportSuccess] = useState(false);

  async function handleFoundUpload(file: File) {
    setLoading(true);
    try {
      const result = await submitFoundUCard(file, note || undefined);
      setSubmitResult(result);
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to submit UCard";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  async function handleReportLost(e: FormEvent) {
    e.preventDefault();
    const cleaned = spireId.replace(/\D/g, "");
    if (cleaned.length !== 8) return;

    setLoading(true);
    try {
      await reportLostUCard(cleaned);
      setReportSuccess(true);
      setSpireId("");
      toast.success("Lost UCard reported successfully!");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to report lost UCard";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
